...
from train_utils.train_3d import mixed_train, mixed_train_sa
from train_utils.train_2d import train_2d_operator, train_2d_operator_sa
from models import FNN3d, FNN2d
from models.basics import SAWeightDarcy, SAWeightNS
...
def train_3d(args, config):
    ...
    if 'self_adaptive' in config['model'].keys() and config['model']['self_adaptive'] is True: 
        weight_model = SAWeightNS(nx=data_config['nx'], ny=data_config['nx'], nt=data_config['nt']).to(device)
        ...
        weight_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                     milestones=config['train']['milestones'],
                                                     gamma=config['train']['scheduler_gamma'])
        mixed_train_sa(model, weight_model, 
                train_loader,
                loader.S, loader.T,
                a_loader,
...
def train_2d(args, config):
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                     milestones=config['train']['milestones'],
                                                     gamma=config['train']['scheduler_gamma'])
    if config['model']['self_adaptive'] is True: 
        ...
        weight_scheduler = torch.optim.lr_scheduler.MultiStepLR(weight_optimizer,
                                                     milestones=config['train']['milestones'],
                                                     gamma=config['train']['scheduler_gamma'])
        train_2d_operator_sa(model, weight_model,
                                    train_loader,
                                    optimizer, weight_optimizer, 
                                    scheduler, weight_scheduler,
