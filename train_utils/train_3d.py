...
def mixed_train(model,              # model of neural operator
                ...
                log=False,          # turn on the wandb # project name      # group name
                tags=['default'],       # tags
...
def mixed_train_cgd(model, discriminator,              # model of neural operator
                train_loader,       # dataloader for training with data
                S1, T1,             # spacial and time dimension for training with data
                a_loader,           # generator for  ICs
                S2, T2,             # spacial and time dimension for training with equation only
                optimizer,          # ACGD optimizer
                config,             # configuration dict
                device=torch.device('cpu'),
    ...
    for ep in pbar:
        ...
        for _ in range(num_data_iter):
            ...
            if len(config['model']['competitive_input']) == 2: 
                out_w = torch.unsqueeze(out, -1)
                x_w = torch.cat((x,out_w), dim=4)
                print(x_w.size())
