...
def train_sa(model, weight_model, 
          loader, train_loader,
          optimizer, scheduler,
    ...
    f_weight = config['train']['f_loss']
    xy_weight = config['train']['xy_loss']
    normalize = config['train']['sa_normalize']
    ...
    for ep in pbar:
        ...
        for x, y in train_loader:
            ...
            if ic_weight != 0 or f_weight != 0:
                loss_ic_w, loss_f_w, loss_ic, loss_f = weight_model(out.view(batch_size, S, S, T),
                                            x, forcing,
                                            v, t_interval) 
            else:
                loss_ic_w, loss_f_w, loss_ic, loss_f = zero, zero, zero, zero
            
            total_loss = loss_l2 * xy_weight + loss_f_w * f_weight + loss_ic_w * ic_weight
